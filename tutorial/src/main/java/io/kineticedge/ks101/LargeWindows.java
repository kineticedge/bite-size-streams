...
public class LargeWindows extends BaseTopologyBuilder {

  private static final Logger log = LoggerFactory.getLogger(LargeWindows.class);
...
//                Map.entry("feature", isFeatureDisabled() ? "disabled" : "enabled"),
  ...
  protected void build(StreamsBuilder builder) {
    ...
    var materialized = Materialized.<String, Long, WindowStore<Bytes, byte[]>>as("count-store")
            ...
            .withCachingDisabled()
            .withRetention(Duration.ofDays(10L));
    ...
    builder
            ...
            .windowedBy(TimeWindows
                    .ofSizeAndGrace(Duration.ofDays(6L), Duration.ofSeconds(1L))
                    .advanceBy(Duration.ofDays(3L))
            ).count(
                    Named.as("count"),
                    materialized
            ).toStream()
            .selectKey((k, v) -> format(k))
            .to(OUTPUT_TOPIC,
                    Produced
