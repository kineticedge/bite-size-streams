...
fetch('/port')
    .then(response => {
        if (!response.ok) {
            throw new Error('Failed to fetch port from server');
        }
        return response.json();
    })
    .then(data => {
        WS_PORT = data.websocketPort;
        console.log("WebSocket port initialized to:", WS_PORT);
    })
    .catch(error => {
        console.error("Error fetching WebSocket port, using default 8081:", error);
        // WS_PORT already has the default value
    });
...
function ws_updateEventSourceDialog(topic, e) {
    ...
    dialog.style.left = `${e.clientX - (650/2)}px`;
