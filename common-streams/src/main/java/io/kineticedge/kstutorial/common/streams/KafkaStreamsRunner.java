...
public class KafkaStreamsRunner {
  ...
  public KafkaStreamsRunner(Integer port, Integer websocketPort, String applicationId, Topology topology, Map<String, Object> config, Map<String, String> metadata, Map<String, String> producerMetadata) {
    this.port = port;
    ...
    streams.setStateListener((newState, oldState) -> {
        ...
        if (newState == KafkaStreams.State.RUNNING) {
          ...
          if (first.compareAndSet(false, true)) {
            ...
            try {
              WebServer server = new WebServer(port, websocketPort, applicationId, topology, streams, metadata, producerMetadata);
              server.start();
            } catch (IOException e) {
              throw new RuntimeException(e);
            }

            SimpleWebSocketServer wsServer = new SimpleWebSocketServer(websocketPort);
            wsServer.start();
